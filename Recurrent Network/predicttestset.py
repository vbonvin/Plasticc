...
'''
picklepath='/scratch/vbonvin/Plasticc/test_samples'
scalerpath='./QuantileTransformer2.pkl'
modelpath='./boostnwelossinvfilter.h5'

saverepertory='./outputtest'
...
for name in picklelist:
	...
	init_data = util.readpickle(picklepath+'/'+name)

	test_data = init_data.loc[:].copy(deep=True)

	[x_test,idn] = dataset_zeropadding(test_data,idnumber=True)
	...
	number_param = 20.
	model = Sequential()
	model.add(Reshape((int(len(x_test[0]) / number_param), int(number_param)), input_shape=(len(x_test[0]),)))
	...
	np.savetxt(saverepertory+'/'+name[12:-4]+'.csv', output,  delimiter=",")
	np.save(saverepertory+'/'+name[12:-4],output) #12fortest
	print ('csv')
